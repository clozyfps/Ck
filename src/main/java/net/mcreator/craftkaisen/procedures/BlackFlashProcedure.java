...
public class BlackFlashProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, Entity entity, Entity sourceentity, double amount) {
		...
		if (sourceentity instanceof Player) {
			...
			if (!(sourceentity instanceof LivingEntity _livEnt ? _livEnt.hasEffect(CraftKaisenModMobEffects.ZONE.get()) : false)) {
				vanim = Mth.nextInt(RandomSource.create(), 1, 10000);
			} else if (sourceentity instanceof LivingEntity _livEnt ? _livEnt.hasEffect(CraftKaisenModMobEffects.ZONE.get()) : false) {
